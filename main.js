...
function getData() {
  axios.all([
    ...
    .then(axios.spresd(axios.spread(todos, posts) => shoeOutput(posts)))
    .catch(err => console.error(err));
}
...
function transformResponse() {
  const options = {
    ...
    transformResponse: axios.defaults.transformRespnse.concat(data => {
      data.title=data.title.toUpperCase();
      return data;
    })
  }
  axios(options).then(rea => showOutput(res))
}
...
function cancelToken() {
  ...
  axios
   ...
   if(true){
     souece.cancel('request cancelled')
   }
